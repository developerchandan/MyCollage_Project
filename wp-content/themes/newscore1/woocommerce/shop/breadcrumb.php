...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
		...
		/* add a shop url to shop category bread crumb */
		echo $before . '<a class="shop_url" href="'.$shop_page_url.'">'.$shop_page_title.'</a>' . $after . $delimiter;
		...
		echo $prepend . $before . __( 'Products tagged &ldquo;', 'radium' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', 'radium' ) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . __( 'Error 404', 'radium' ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', 'radium' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', 'radium' ) . single_tag_title('', false) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo $before . __( 'Author:', 'radium' ) . ' ' . $userdata->display_name . $after;

	}

	if ( get_query_var( 'paged' ) )
		echo ' (' . __( 'Page', 'radium' ) . ' ' . get_query_var( 'paged' ) . ')';
