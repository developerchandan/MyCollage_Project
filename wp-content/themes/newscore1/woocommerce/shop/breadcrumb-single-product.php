...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( get_post_type() == 'product' ) {
			...
			if ( ! empty( $home ) ) {
				echo $before . '<a class="home" href="'.$shop_page_url.'">'.$shop_page_title.'</a>' . $after . $delimiter;
			}

			echo $prepend;

			if ( $terms = wp_get_post_terms( get_the_ID(), 'product_cat', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {
