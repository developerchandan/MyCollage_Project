 ...
 * @return void
 */
function radium_breadcrumb_trail( $args = array() ) {
	
	if ( function_exists( 'is_buddypress' ) && is_buddypress() )
		$breadcrumb = new Radium_BuddyPress_Breadcrumb_Trail( $args );
	elseif ( function_exists( 'is_bbpress' ) && is_bbpress() )
		$breadcrumb = new Radium_bbPress_Breadcrumb_Trail( $args );
	else
		$breadcrumb = new Radium_Breadcrumb_Trail( $args );
...
class Radium_Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'radium_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function trail() {
		...
		if ( !empty( $this->items ) && is_array( $this->items ) ) {
			...
			$breadcrumb = "\n\t\t" . '<' . tag_escape( $this->args['container'] ) . ' class="breadcrumb-trail breadcrumbs" itemscope itemtype="http://data-vocabulary.org/Breadcrumb" itemprop="breadcrumb">';
		...
		$breadcrumb = apply_filters( 'radium_breadcrumb_trail', $breadcrumb, $this->args );
	...
	public function do_trail_items() {
		...
		$this->items = apply_filters( 'radium_breadcrumb_trail_items', $this->items, $this->args );
	}
	...
	public function do_network_home_link() {
		if ( is_multisite() && !is_main_site() && true === $this->args['network'] )
			$this->items[] = '<a href="' . network_home_url() . '" title="' . esc_attr( $this->args['labels']['home'] ) . '" rel="home"  itemprop="url"><span itemprop="title">' . $this->args['labels']['home'] . '</span></a>';
	}
	...
	public function do_site_home_link() {
		...
		$this->items[] = '<a href="' . home_url() . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '"' . $rel .'  itemprop="url"><span itemprop="title">' . $label . '</span></a>';
	}
	...
	public function do_posts_page_items() {
		...
		if ( is_paged() )
			$this->items[]  = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $title ) . '" itemprop="url"><span itemprop="title">' . $title . '</span></a>';
	...
	public function do_singular_items() {
		...
		if ( $post_title = single_post_title( '', false ) ) {
			...
			if ( 1 < get_query_var( 'page' ) )
				$this->items[] = '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $post_title ) . '" itemprop="url"><span itemprop="title">' . $post_title . '</span></a>';
	...
	public function do_post_hierarchy( $post_id ) {
		...
		if ( !empty( $post_type_object->has_archive ) ) {
			...
			$this->items[] = '<a href="' . get_post_type_archive_link( $post_type ) . '" itemprop="url"><span itemprop="title">' . $label . '</span></a>';
		}
	}
	...
	public function do_term_archive_items() {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						if ( !empty( $post_types ) ) {
							...
							$this->items[] = '<a href="' . get_post_type_archive_link( $post_type_object->name ) . '" title="' . esc_attr( $label ) . '" itemprop="url"><span itemprop="title">' . $label . '</span></a>';
		...
		if ( is_paged() )
			$this->items[] = '<a href="' . esc_url( get_term_link( $term, $term->taxonomy ) ) . '" title="' . esc_attr( single_term_title( '', false ) ) . '" itemprop="url"><span itemprop="title">' . single_term_title( '', false ) . '</span></a>';
	...
	public function do_post_type_archive_items() {
		...
		if ( is_paged() )
			$this->items[] = '<a href="' . esc_url( get_post_type_archive_link( $post_type_object->name ) ) . '" title="' . esc_attr( post_type_archive_title( '', false ) ) . '" itemprop="url"><span itemprop="title">' . post_type_archive_title( '', false ) . '</span></a>';
	...
	public function do_user_archive_items() {
		...
		if ( is_paged() )
			$this->items[] = '<a href="'. esc_url( get_author_posts_url( $user_id ) ) . '" title="' . esc_attr( get_the_author_meta( 'display_name', $user_id ) ) . '" itemprop="url"><span itemprop="title">' . get_the_author_meta( 'display_name', $user_id ) . '</span></a>';
	...
	public function do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'radium' ) ) );
	}
	...
	public function do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'radium' ) ) );
	}
	...
	public function do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'radium' ) ) );
	}
	...
	public function do_day_archive_items() {
		...
		$year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format',  'radium' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'radium' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( _x( 'j', 'daily archives date format',   'radium' ) ) );

		/* Add the year and month items. */
		$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( $year ) . '" itemprop="url"><span itemprop="title">' . $year . '</span></a>';
		$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ) . '" title="' . esc_attr( $month ) . '" itemprop="url"><span itemprop="title">' . $month . '</span></a>';

		/* Add the day item. */
		if ( is_paged() )
			$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y' ), get_the_time( 'm' ), get_the_time( 'd' ) ) . '" title="' . esc_attr( $day ) . '" itemprop="url"><span itemprop="title">' . $day . '</span></a>';
	...
	public function do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format', 'radium' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'],  get_the_time( _x( 'W', 'weekly archives date format', 'radium' ) ) );
	...
	public function do_search_items() {
		...
		if ( is_paged() )
			$this->items[] = '<a href="' . get_search_link() . '" title="' . esc_attr( sprintf( $this->args['labels']['search'], get_search_query() ) ) . '" itemprop="url"><span itemprop="title">' . sprintf( $this->args['labels']['search'], get_search_query() ) . '</span></a>';
	...
	function do_term_parents( $term_id, $taxonomy ) {
		...
		while ( $term_id ) {
			...
			$parents[] = '<a href="' . get_term_link( $term, $taxonomy ) . '" title="' . esc_attr( $term->name ) . '" itemprop="url"><span itemprop="title">' . $term->name . '</span></a>';
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" itemprop="url"><span itemprop="title">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format',  'radium' ) ) ) . '</span></a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" itemprop="url"><span itemprop="title">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'radium' ) ) ) . '</span></a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" itemprop="url"><span itemprop="title">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'radium' ) ) ) . '</span></a>';

				/* If using the %author% tag, add a link to the post author archive. */
				elseif ( '%author%' == $tag )
					$this->items[] = '<a href="' . get_author_posts_url( $post->post_author ) . '" title="' . esc_attr( get_the_author_meta( 'display_name', $post->post_author ) ) . '" itemprop="url"><span itemprop="title">' . get_the_author_meta( 'display_name', $post->post_author ) . '</span></a>';
				...
				elseif ( '%category%' == $tag ) {
					...
					if ( $terms ) {
						...
						$this->items[] = '<a href="' . get_term_link( $term, 'category' ) . '" title="' . esc_attr( $term->name ) . '" itemprop="url"><span itemprop="title">' . $term->name . '</span></a>';
					}
				}
			}
		}
 ...
 * @access public
 */
class Radium_bbPress_Breadcrumb_Trail extends Radium_Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		if ( !empty( $post_type_object->has_archive ) && !bbp_is_forum_archive() )
			$this->items[] = '<a href="' . get_post_type_archive_link( bbp_get_forum_post_type() ) . '" itemprop="url"><span itemprop="title">' . bbp_get_forum_archive_title() . '</span></a>';
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			$this->items[] = '<a href="' . bbp_get_topic_tag_link() . '" itemprop="url"><span itemprop="title">' . bbp_get_topic_tag_name() . '</span></a>';

			if ( true === $this->args['show_title'] )
				$this->items[] = __( 'Edit', 'radium' );
		}
		...
		elseif ( bbp_is_single_topic() ) {
			...
			if ( bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit() )
				$this->items[] = '<a href="' . bbp_get_topic_permalink( $topic_id ) . '" itemprop="url"><span itemprop="title">' . bbp_get_topic_title( $topic_id ) . '</span></a>';
		...
		elseif ( bbp_is_single_reply() ) {
			...
			if ( bbp_is_reply_edit() ) {
				$this->items[] = '<a href="' . bbp_get_reply_url( $reply_id ) . '" itemprop="url"><span itemprop="title">' . bbp_get_reply_title( $reply_id ) . '</span></a>';
		...
		elseif ( bbp_is_single_user() || bbp_is_single_user_edit() ) {
			...
			if ( bbp_is_single_user_edit() ) {
				$this->items[] = '<a href="' . bbp_get_user_profile_url() . '" itemprop="url"><span itemprop="title">' . bbp_get_displayed_user_field( 'display_name' ) . '</span></a>';
		...
		$this->items = apply_filters( 'radium_breadcrumb_trail_get_bbpress_items', $this->items, $this->args );
	}
}

/**
 * Extends the Breadcrumb_Trail class for bbPress.  Only use this if bbPress is in use.
 *
 * @since  0.6.0
 * @access public
 * @author Franklin Gitonga
 */
class Radium_BuddyPress_Breadcrumb_Trail extends Radium_Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		/**
		* Show breadcrumbs on all member components.
		*/ 			
		if( bp_is_activity_directory() ) : 
			$this->do_activity_link();
		endif;
		...
		*/ 			
		if( bp_is_members_directory() ) : 
			$this->do_members_link();
		endif;
		
		/**
		* Show breadcrumbs on all groups components.
		*/ 			
		...
		*/
		...
		/* Return the bbPress breadcrumb trail items. */
		$this->items = apply_filters( 'radium_breadcrumb_trail_get_buddypress_items', $this->items, $this->args );
	}
	
	/**
	 * Adds the member page link to the items array
	 *
	 * @since  0.6.0
	 * @access public
	 * @return void
	 */
	public function do_activity_link() {
		
		global $bp;
				
		$this->items[] = '<a href="' . $bp->root_domain . '/' .$bp->activity->root_slug . '" title="' . $bp->activity->root_slug . '" rel="home" itemprop="url"><span itemprop="title">' . $bp->activity->root_slug . '</span></a>'; 
	 ...
	 * @return void
	 */
	public function do_members_link() {
		
		global $bp;
		
		$members_slug = $bp->members->root_slug; // slug for the Members page. The BuddyPress default is 'members'. 
		
		$this->items[] = '<a href="' . $bp->root_domain . '/' .$bp->members->root_slug . '" title="' . $bp->members->root_slug . '" rel="home" itemprop="url"><span itemprop="title">' . $bp->members->root_slug . '</span></a>'; 
		
	}
	
	/**
	 * Adds the groups page link to the items array
	 *
	 * @since  0.6.0
	 * @access public
	 * @return void
	 */
	public function do_groups_link() {
		
		global $bp;
		
		$members_slug = $bp->groups->root_slug; // slug for the Members page. The BuddyPress default is 'groups'. 
		
		$this->items[] = '<a href="' . $bp->root_domain . '/' .$bp->groups->root_slug . '" title="' . $bp->groups->root_slug . '" rel="home" itemprop="url"><span itemprop="title">' . $bp->groups->root_slug . '</span></a>'; 
		
	}
	
	/**
	 * Adds the user page link to the items array
	 *
	 * @since  0.6.0
	 * @access public
	 * @return void
	...
	public function do_user_link() {
		...
		$this->items[] = '<a href="' . $bp->displayed_user->domain . '" itemprop="url"><span itemprop="title">' . $bp->displayed_user->userdata->display_name . '</span></a>';
	}
	 ...
	 * @return void
	 */
	public function do_registration_link() {
				
		$this->items[] = '<a href="' . get_permalink() . '" title="' . $title . '" itemprop="url"><span itemprop="title">' . $title . '</span></a>';
	}
}
