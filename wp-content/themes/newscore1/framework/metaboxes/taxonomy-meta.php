...
class RW_Taxonomy_Meta {
	...
	function check_field_upload() {
		...
		if ( $this->has_field( 'file' ) ) {
			...
			\$('body').on('click', '.rwtm-file-upload', function(){
				...
				template += ' (<a class=\"rwtm-delete-file\" href=\"#\">" . __( 'Delete', 'radium' ) . "</a>)';
	...
	function show_field_file( $field, $meta ) {
		...
		echo "<a href='#' class='rwtm-file-upload button' data-field='{$field['id']}'>" . __( 'Select File', 'radium' ) . "</a>";
		echo '</td>';
	}
	...
	function show_field_image( $field, $meta ) {
		...
		echo "<a href='#' class='rwtm-image-upload button' data-field='{$field['id']}'>" . __( 'Select Image', 'radium' ) . "</a>";
		echo '</td>';
	}
