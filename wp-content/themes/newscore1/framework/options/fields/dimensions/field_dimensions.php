<?php
    ...
    if ( ! class_exists( 'Radium_Options_dimensions' ) ) {
        class Radium_Options_dimensions {
            ...
            function render() {
                ...
                if ( isset( $this->field['units'] ) && ! Radium_Options::array_in_array( $this->field['units'], array(
                            '',
                            false,
                            '%',
                            'in',
                ...
                if ( isset( $this->value['units'] ) && ! Radium_Options::array_in_array( $this->value['units'], array(
                            '',
                            '%',
                            'in',
                            'cm',
                ...
                echo '<fieldset id="' . $this->field['id'] . '" class="radium-opts-dimensions-container" data-id="' . $this->field['id'] . '">';
                ...
                if ( $this->field['width'] === true ) {
                    ...
                    echo '<input type="text" class="radium-opts-dimensions-input radium-opts-dimensions-width mini' . $this->field['class'] . '" placeholder="' . __( 'Width', 'radium' ) . '" rel="' . $this->field['id'] . '-width" value="' . filter_var( $this->value['width'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) . '">';
                    echo '<input data-id="' . $this->field['id'] . '" type="hidden" id="' . $this->field['id'] . '-width" 
                    name="'.$this->parent->args['opt_name'].'['.$this->field['id'].'][width]' . $this->field['name_suffix'] . '" value="' . $this->value['width'] . '"></div>';
                }
                ...
                if ( $this->field['height'] === true ) {
                    ...
                    echo '<input type="text" class="radium-opts-dimensions-input radium-opts-dimensions-height mini' . $this->field['class'] . '" placeholder="' . __( 'Height', 'radium' ) . '" rel="' . $this->field['id'] . '-height" value="' . filter_var( $this->value['height'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION ) . '">';
                    echo '<input data-id="' . $this->field['id'] . '" type="hidden" id="' . $this->field['id'] . '-height" 
                    name="'.$this->parent->args['opt_name'].'['.$this->field['id'].'][height]' . $this->field['name_suffix'] . '" value="' . $this->value['height'] . '"></div>';
                }
                ...
                if ( isset( $this->field['units'] ) && $this->field['units'] !== false ) {
                    echo '<div class="select_wrapper dimensions-units" original-title="' . __( 'Units', 'radium' ) . '">';
                    echo '<select data-id="' . $this->field['id'] . '" data-placeholder="' . __( 'Units', 'radium' ) . '" class="radium-opts-dimensions radium-opts-dimensions-units select' . $this->field['class'] . '" original-title="' . __( 'Units', 'radium' ) . '" name="'.$this->parent->args['opt_name'].'['.$this->field['id'].'][units]' . $this->field['name_suffix'] . '">';
            ...
            function enqueue() {
                wp_enqueue_script(
                    'radium-opts-field-dimensions-js',
                    $this->parent->url . 'fields/dimensions/field_dimensions.js',
                    array( 'jquery', 'select2-js', 'radium-opts-js' ),
                    time(),
                    true
                );

                wp_enqueue_style(
                    'radium-opts-field-dimensions-css',
                    $this->parent->url . 'fields/dimensions/field_dimensions.css',
                    time(),
                    true
                );
                
                wp_enqueue_style( 'radium-opts-elusive-icon');
                wp_enqueue_style( 'radium-opts-elusive-icon-ie7');
			}
