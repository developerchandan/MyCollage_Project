...
class Radium_Options_radio_img extends Radium_Options{	
	...
	function render(){
		...
		echo '<fieldset>';
			...
			foreach($this->field['options'] as $k => $v){
				...
				echo '<img src="'.$v['img'].'" alt="'.$v['title'].'" title="'.$v['title'].'" onclick="jQuery:radium_radio_img_select(\''.$this->field['id'].'_'.array_search($k,array_keys($this->field['options'])).'\', \''.$this->field['id'].'\');"  rel="tooltip" />';
	...
	*/
	function enqueue(){
		...
		wp_enqueue_script( 'radium-opts-field-radio_img-js', RADIUM_OPTIONS_URL.'fields/radio_img/field_radio_img.js', array('jquery'), time(), true );
